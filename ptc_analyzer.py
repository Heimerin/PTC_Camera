...
SUPPORTED_EXTENSIONS = {
    ".fit", ".fits", ".fts", ".tif", ".tiff", ".png", ".jpg", ".jpeg", ".npy",
}

EXPOSURE_HEADER_KEYS = (
    "EXPTIME", "EXPOSURE", "EXP_TIME", "EXP", "ITIME", "INTTIME", "EXPOS",
)
...
def extract_color_plane(data: np.ndarray, color_mode: str) -> np.ndarray:
    ...
    if data.ndim >= 3:
        if mode in {"first", "channel_0"}: return np.asarray(data[..., 0], dtype=np.float64)
        if mode in {"red", "rgb_red", "r"}: return np.asarray(data[..., 0], dtype=np.float64)
        if mode in {"green", "rgb_green", "g"}: return np.asarray(data[..., 1], dtype=np.float64)
        if mode in {"blue", "rgb_blue", "b"}: return np.asarray(data[..., 2], dtype=np.float64)
    ...
    bayer_offsets = {
        "bayer_rggb_red": (0, 0), "bayer_rggb_green1": (0, 1),
        "bayer_rggb_green2": (1, 0), "bayer_rggb_blue": (1, 1),
        "bayer_bggr_red": (1, 1), "bayer_bggr_green1": (0, 1),
        "bayer_bggr_green2": (1, 0), "bayer_bggr_blue": (0, 0),
        "bayer_grbg_red": (0, 1), "bayer_grbg_green1": (0, 0),
        "bayer_grbg_green2": (1, 1), "bayer_grbg_blue": (1, 0),
        "bayer_gbrg_red": (1, 0), "bayer_gbrg_green1": (0, 0),
        "bayer_gbrg_green2": (1, 1), "bayer_gbrg_blue": (0, 1),
    }
...
def read_image(path: Path, roi: Optional[Tuple[int, int, int, int]] = None, color_mode: str = "mono") -> Tuple[np.ndarray, Dict[str, object]]:
...
def extract_exposure_seconds(path: Path, header: Dict[str, object], exposure_regex: Optional[str] = None) -> float:
    ...
    for pattern in regexes:
        ...
        if match:
            ...
            if unit == "ms":
                return value / 1000.0
            return value
    raise ValueError(f"Could not determine exposure time for {path.name}.")

def robust_std(values: np.ndarray) -> float:
    vals = np.asarray(values, dtype=np.float64)
    vals = vals[np.isfinite(vals)]
    if vals.size < 2: return float("nan")
    med = np.median(vals)
    mad = np.median(np.abs(vals - med))
    if mad > 0: return float(1.4826 * mad)
    return float(np.std(vals, ddof=1))

def mean_stack(paths: Sequence[Path], roi: Optional[Tuple[int, int, int, int]], color_mode: str) -> np.ndarray:
    if not paths: raise ValueError("No image files found")
    acc = None
...
def estimate_read_noise_from_biases(bias_files: Sequence[Path], master_bias: np.ndarray, roi: Optional[Tuple[int, int, int, int]], color_mode: str) -> float:
    if len(bias_files) >= 2:
        noises = []
...
def group_flats_by_exposure(flat_files: Sequence[Path], roi: Optional[Tuple[int, int, int, int]], exposure_regex: Optional[str]) -> Dict[float, List[Tuple[Path, Dict[str, object]]]]:
...
def estimate_ptc_turnover(signal: np.ndarray, read_shot_noise: np.ndarray) -> float:
    ...
    if np.count_nonzero(mask) == 0: return float("nan")
    sig = signal[mask]
    ...
    if peak_idx < len(sig) - 1: return float(sig[peak_idx])
...
def save_csv(path: Path, points: Sequence[PTCPoint]) -> None:
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        if points:
            writer.writeheader()
            for p in points: writer.writerow(asdict(p))
...
def setup_plot_style() -> None:
    plt.rcParams.update({
        "figure.dpi": 150, "savefig.dpi": 150, "axes.grid": True, "grid.alpha": 0.25,
...
def plot_ptc(results: PTCResults, output_dir: Path) -> None:
    ...
    if not np.any(below_fwc): below_fwc = np.ones_like(signal_adu, dtype=bool)
...
def plot_k_fit(results: PTCResults, output_dir: Path) -> None:
    setup_plot_style()
    pts = results.points
    signal = np.array([p.signal_dn for p in pts])
    k = np.array([p.k_adc_e_per_dn_point for p in pts])
    ...
    fig, ax = plt.subplots(figsize=(8.5, 5.5), layout="constrained")
    ax.plot(signal, k, "o", color="#20808D", label="Raw K_ADC")
    
    if results.k_fit_coeffs and len(results.k_fit_coeffs) > 0 and not np.all(np.array(results.k_fit_coeffs) == 0):
        xs = np.linspace(np.nanmin(signal), np.nanmax(signal), 200)
        ys = np.polyval(results.k_fit_coeffs, xs)
        ax.plot(xs, ys, "-", color="#A84B2F", label="K_fit polynomial")
        ax.axvline(results.k_fit_low_adu, color="#7A7974", ls=":", label="Plateau start")
        ax.axvline(results.k_fit_high_adu, color="#7A7974", ls=":", label="Plateau end")
        
    ax.axhline(results.k_low_e_per_dn, color="#1B474D", lw=1.5, ls="--", label=f"K_low = {results.k_low_e_per_dn:.4g}")
    ax.set_title(f"K_ADC Fit Curve: {results.camera_name}")
    ax.set_xlabel("Signal [ADU]")
    ax.set_ylabel("K_ADC [e-/ADU]")
    ax.legend(loc="best")
    
    fig.savefig(output_dir / "k_fit_curve.png", bbox_inches="tight")
    plt.close(fig)
...
def write_markdown_report(results: PTCResults, output_dir: Path) -> None:
    ...
    lines = [
        ...
        f"- Selected K_ADC plateau range: {results.k_fit_low_adu:.6g} ADU to {results.k_fit_high_adu:.6g} ADU",
        f"- ADC sensitivity K_ADC: {results.k_adc_e_per_dn:.6g} e-/ADU",
        f"- Reference K_low (start of plateau): {results.k_low_e_per_dn:.6g} e-/ADU",
        ...
        "- K nonlinearity is computed relative to the start of the user-selected plateau: `100 * (K_LOW - K) / K_LOW`.",
        ...
        "- `k_calibration.json`: K_ADC polynomial calibration profile.",
        "- `ptc_loglog.png`: classical PTC in electron units.",
        "- `variance_fit.png`: variance PTC and fit.",
        "- `k_fit_curve.png`: K_ADC polynomial fit visualizer.",
        "- `nonlinearity.png`: K_ADC and nonlinearity curves plotted over the entire sensor range.",
...
def analyze_camera(config: AnalysisConfig) -> PTCResults:
    ...
    if not raw_points:
        raise ValueError("No flat pairs were available.")
    ...
    if len(valid_indices) > 0:
        ...
        k_low = k_points[first_idx]
    else:
        k_low = float("nan")

    k_fit_mask = signal <= high_val
    if np.count_nonzero(k_fit_mask) > 3:
        k_fit_coeffs = np.polyfit(signal[k_fit_mask], k_points[k_fit_mask], 3)
    else:
        k_fit_coeffs = np.zeros(4)

    calibration_data = {
        "camera_name": config.camera_name,
        "k_low": float(k_low),
        "k_fit_coeffs": k_fit_coeffs.tolist(),
        "plateau_range": [float(low_val), float(high_val)]
    }
...
#to tu tez
    ...
    results = PTCResults(
        ...
        k_fit_low_adu=float(low_val),
        k_fit_high_adu=float(high_val),
...
def run_gui() -> None:
    ...
    color_modes = [
        "mono", "red", "green", "blue", "luminance",
        "bayer_rggb_red", "bayer_bggr_red", "bayer_grbg_red", "bayer_gbrg_red",
        "bayer_rggb_green1", "bayer_rggb_green2", "bayer_rggb_blue",
        "bayer_bggr_green1", "bayer_bggr_green2", "bayer_bggr_blue",
        "bayer_grbg_green1", "bayer_grbg_green2", "bayer_grbg_blue",
        "bayer_gbrg_green1", "bayer_gbrg_green2", "bayer_gbrg_blue",
    ]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--color-mode",
        default="mono",
        help="Color extraction mode.",
    )
    parser.add_argument("--exposure-regex", default=None, help="Custom regex for exposure.")
    return parser
